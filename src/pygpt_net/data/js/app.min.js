...
async renderAsync(root){if(typeof katex==='undefined')return;const scope=root||document;const scripts=Array.from(scope.querySelectorAll('script[type^="math/tex"]'));const useToString=(typeof katex.renderToString==='function');const decodeEscapedMathText=(raw)=>String(raw??'').replace(/&(amp|lt|gt);/g,(_match,entity)=>entity==='amp'?'&':(entity==='lt'?'<':'>'));const renderWithoutNeqMetricsWarning=(fn)=>{if(typeof console==='undefined'||typeof console.warn!=='function')return fn();const originalWarn=console.warn;console.warn=function(...args){const msg=args.length>0?String(args[0]):'';if(msg==="No character metrics for '≠' in style 'Main-Regular' and mode 'math'")return;return originalWarn.apply(this,args);};try{return fn();}finally{console.warn=originalWarn;}};const batchFn=async(script)=>{if(!script||!script.isConnected)return;if(!script.closest('.msg-box.msg-bot'))return;const t=script.getAttribute('type')||'';const displayMode=t.indexOf('mode=display')>-1;const mathContent=decodeEscapedMathText(script.textContent||'');const parent=script.parentNode;if(!parent)return;try{if(useToString){let html='';try{html=renderWithoutNeqMetricsWarning(()=>katex.renderToString(mathContent,{displayMode,throwOnError:false,macros:{"\\neq":"\\mathrel{\\char\"2260}","\\ne":"\\mathrel{\\char\"2260}"}}));}catch(_){const fb=displayMode?`\\[${mathContent}\\]`:`\\(${mathContent}\\)`;html=(displayMode?`<div>${Utils.escapeHtml(fb)}</div>`:`<span>${Utils.escapeHtml(fb)}</span>`);}
const host=document.createElement(displayMode?'div':'span');host.innerHTML=html;const el=host.firstElementChild||host;if(parent.classList&&parent.classList.contains('math-pending'))parent.replaceWith(el);else parent.replaceChild(el,script);}else{const el=document.createElement(displayMode?'div':'span');try{renderWithoutNeqMetricsWarning(()=>katex.render(mathContent,el,{displayMode,throwOnError:false,macros:{"\\neq":"\\mathrel{\\char\"2260}","\\ne":"\\mathrel{\\char\"2260}"}}));}catch(_){el.textContent=(displayMode?`\\[${mathContent}\\]`:`\\(${mathContent}\\)`);}
...
reset(){this._d('reset',{});this._clearStreamBuffer();this.fenceOpen=false;this.fenceMark='`';this.fenceLen=3;this.fenceTail='';this.fenceBuf='';this.lastSnapshotTs=0;this.nextSnapshotStep=this.profile().base;this.snapshotScheduled=false;this.snapshotRAF=0;this.codeStream={open:false,lines:0,chars:0};this.activeCode=null;this.suppressPostFinalizePass=false;this._promoteScheduled=false;this._firstCodeOpenSnapDone=false;this._lastInjectedEOL=false;this._fenceCustom=null;this.reasoningThinking=false;this.reasoningVisible=false;this.reasoningHasResponseText=false;this.reasoningFadeInStartedAt=0;this.reasoningFadeOutStartedAt=0;this._cancelReasoningTimers();this._plainReset();}
_cancelReasoningTimers(){try{if(this.reasoningHideDelayTimer)clearTimeout(this.reasoningHideDelayTimer);if(this.reasoningFadeOutTimer)clearTimeout(this.reasoningFadeOutTimer);}catch(_){}
this.reasoningHideDelayTimer=0;this.reasoningFadeOutTimer=0;}
_updateReasoningVisibilityFromChunk(chunk){const s=String(chunk||'');if(!s)return{changed:false,hasResponseText:false};const beforeThinking=!!this.reasoningThinking;let thinking=beforeThinking;let hasResponseText=false;let pos=0;while(pos<s.length){const openAt=s.indexOf('<think>',pos);const closeAt=s.indexOf('</think>',pos);let nextAt=-1;let isOpen=false;if(openAt!==-1&&(closeAt===-1||openAt<closeAt)){nextAt=openAt;isOpen=true;}else if(closeAt!==-1){nextAt=closeAt;}
if(nextAt===-1){if(!thinking&&s.slice(pos).trim()!=='')hasResponseText=true;break;}
if(!thinking&&s.slice(pos,nextAt).trim()!=='')hasResponseText=true;if(isOpen){thinking=true;pos=nextAt+7;}else{thinking=false;pos=nextAt+8;}}
this.reasoningThinking=thinking;if(hasResponseText)this.reasoningHasResponseText=true;if(!beforeThinking&&thinking){this._cancelReasoningTimers();this.reasoningFadeOutStartedAt=0;if(this.reasoningEnabled){const now=(typeof performance!=='undefined'&&typeof performance.now==='function')?performance.now():Date.now();if(!this.reasoningVisible)this.reasoningFadeInStartedAt=now;this.reasoningVisible=true;}else{this.reasoningVisible=false;}}
return{changed:beforeThinking!==thinking,hasResponseText:hasResponseText};}
_scheduleReasoningHide(msg){if(!this.reasoningEnabled||!this.reasoningHideAfterResponse||!this.reasoningVisible||this.reasoningThinking)return;if(this.reasoningHideDelayTimer||this.reasoningFadeOutTimer||this.reasoningFadeOutStartedAt>0)return;const startFade=()=>{this.reasoningHideDelayTimer=0;if(this.reasoningThinking||!this.reasoningVisible)return;const duration=Math.max(0,Number(this.reasoningFadeDuration)||0);this.reasoningVisible=false;this.reasoningFadeInStartedAt=0;this.reasoningFadeOutStartedAt=(typeof performance!=='undefined'&&typeof performance.now==='function')?performance.now():Date.now();const root=this.getMsgSnapshotRoot(msg);this._syncReasoningVisibility(root);if(duration<=0){this.reasoningFadeOutStartedAt=0;this._syncReasoningVisibility(this.getMsgSnapshotRoot(msg));return;}
this.reasoningFadeOutTimer=setTimeout(()=>{this.reasoningFadeOutTimer=0;if(this.reasoningThinking)return;this.reasoningFadeOutStartedAt=0;this._syncReasoningVisibility(this.getMsgSnapshotRoot(msg));},duration+24);};const delay=Math.max(0,Number(this.reasoningFadeOutDelay)||0);if(delay<=0)startFade();else this.reasoningHideDelayTimer=setTimeout(startFade,delay);}
_syncReasoningVisibility(root){try{if(!root||typeof root.querySelectorAll!=='function')return;const nodes=root.querySelectorAll('think');if(!nodes||!nodes.length)return;if(!this.reasoningEnabled){for(const el of nodes){if(!el||!el.style)continue;if(el.dataset)el.dataset.streamReasoningHidden='1';el.style.removeProperty('height');el.style.removeProperty('overflow');el.style.removeProperty('transition');el.style.setProperty('opacity','0');el.style.setProperty('display','none','important');}return;}const now=typeof performance!=='undefined'&&typeof performance.now==='function'?performance.now():Date.now();const duration=Math.max(0,Number(this.reasoningFadeDuration)||0);const activeIndex=this.reasoningVisible?nodes.length-1:-1;const latestIndex=nodes.length-1;const hideElapsed=this.reasoningFadeOutStartedAt>0?Math.max(0,now-this.reasoningFadeOutStartedAt):duration;const slideUpActive=duration>0&&!this.reasoningVisible&&this.reasoningFadeOutStartedAt>0&&hideElapsed<duration;for(let i=0;i<nodes.length;i++){const el=nodes[i];if(!el||!el.style)continue;if(i===activeIndex){if(el.dataset){delete el.dataset.streamReasoningHidden;delete el.dataset.streamReasoningSliding;}el.style.removeProperty('display');el.style.removeProperty('height');el.style.removeProperty('overflow');const elapsed=this.reasoningFadeInStartedAt>0?Math.max(0,now-this.reasoningFadeInStartedAt):duration;if(duration>0&&elapsed<duration){const remaining=Math.max(1,duration-elapsed);const opacity=Math.min(1,Math.max(0,elapsed/duration));el.style.setProperty('opacity',String(opacity));el.style.setProperty('transition',`opacity ${remaining}ms ease`);requestAnimationFrame(()=>{try{if(this.reasoningVisible&&el.isConnected)el.style.setProperty('opacity','1');}catch(_){}});}else{el.style.setProperty('opacity','1');el.style.removeProperty('transition');}}else if(i===latestIndex&&slideUpActive){if(el.dataset)el.dataset.streamReasoningHidden='sliding';el.style.removeProperty('display');el.style.setProperty('opacity','1');if(!el.dataset||el.dataset.streamReasoningSliding!=='1'){const remaining=Math.max(1,duration-hideElapsed);let height=0;try{height=Math.max(0,el.getBoundingClientRect().height||el.scrollHeight||0);}catch(_){}if(el.dataset)el.dataset.streamReasoningSliding='1';el.style.setProperty('overflow','hidden');el.style.setProperty('height',`${height}px`);el.style.setProperty('transition','none');void el.offsetHeight;requestAnimationFrame(()=>{try{if(this.reasoningVisible||!el.isConnected)return;el.style.setProperty('transition',`height ${remaining}ms ease`);el.style.setProperty('height','0px');}catch(_){}});}}else{if(el.dataset){el.dataset.streamReasoningHidden='1';delete el.dataset.streamReasoningSliding;}el.style.removeProperty('height');el.style.removeProperty('overflow');el.style.removeProperty('transition');el.style.setProperty('opacity','0');el.style.setProperty('display','none','important');}}}catch(_){}}
...
const MAX_CMP=6;const eq=(a,b)=>{try{if(!a||!b)return false;if(a.nodeType!==b.nodeType)return false;if(a.nodeType===3||a.nodeType===8)return a.nodeValue===b.nodeValue;if(a.nodeType===1){const ae=a,be=b;if(ae.tagName!==be.tagName)return false;const acls=ae.className||'';if(acls!==(be.className||''))return false;if(ae.tagName==='THINK')return ae.textContent===be.textContent;return ae.isEqualNode(be);}
...
this.preserveStableClosedCodes(snap,frag,this.fenceOpen===true);this._patchSnapshotRoot(snap,frag);this._syncReasoningVisibility(snap);try{if(this.highlighter&&typeof this.highlighter.microHighlightNow==='function'){this.highlighter.microHighlightNow(snap,{maxCount:1,budgetMs:4},this.activeCode);}}catch(_){}
...
endStream(){this.isStreaming=false;const msg=this.getMsg(false,'');if(msg)this.renderSnapshot(msg);if(!this.reasoningHideAfterResponse&&this.reasoningHasResponseText){this._cancelReasoningTimers();this.reasoningVisible=false;this.reasoningFadeInStartedAt=0;this.reasoningFadeOutStartedAt=0;if(msg)this._syncReasoningVisibility(this.getMsgSnapshotRoot(msg));}this.snapshotScheduled=false;try{this.raf.cancel('SE:snapshot');}catch(_){}
...
if(this.snapshotScheduled&&!this.raf.isScheduled('SE:snapshot'))this.snapshotScheduled=false;const msg=this.getMsg(true,name_header);if(!msg||!chunk)return;const s=String(chunk);const reasoningState=this._updateReasoningVisibilityFromChunk(s);if(reasoningState.hasResponseText&&!this.reasoningThinking){this._scheduleReasoningHide(msg);}
if(/[<>]/.test(s)){this._d('apply.chunk',{len:s.length,nl:Utils.countNewlines(s),head:s.slice(0,120),tail:s.slice(-120)});}
