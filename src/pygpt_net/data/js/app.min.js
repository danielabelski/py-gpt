...
async renderAsync(root){if(typeof katex==='undefined')return;const scope=root||document;const scripts=Array.from(scope.querySelectorAll('script[type^="math/tex"]'));const useToString=(typeof katex.renderToString==='function');const decodeEscapedMathText=(raw)=>String(raw??'').replace(/&(amp|lt|gt);/g,(_match,entity)=>entity==='amp'?'&':(entity==='lt'?'<':'>'));const batchFn=async(script)=>{if(!script||!script.isConnected)return;if(!script.closest('.msg-box.msg-bot'))return;const t=script.getAttribute('type')||'';const displayMode=t.indexOf('mode=display')>-1;const mathContent=decodeEscapedMathText(script.textContent||'');const parent=script.parentNode;if(!parent)return;try{if(useToString){let html='';try{html=katex.renderToString(mathContent,{displayMode,throwOnError:false});}catch(_){const fb=displayMode?`\\[${mathContent}\\]`:`\\(${mathContent}\\)`;html=(displayMode?`<div>${Utils.escapeHtml(fb)}</div>`:`<span>${Utils.escapeHtml(fb)}</span>`);}
